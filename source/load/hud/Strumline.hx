...
class Strumline extends FlxGroup
{
	...
	public function addSplash(note:Note)
	{
		...
		var pref:String = '-' + Configs.getDirection(note.noteData) + '-' + note.strumlineID;
	...
	*/
	public function updateHitbox()
	{
		for(strum in strumGroup)
		{
			...
			strum.x = x;
			strum.x += Configs.noteWidth() * strum.strumData;
			
			strum.x -= (Configs.noteWidth() * (strumGroup.members.length - 1)) / 2;
