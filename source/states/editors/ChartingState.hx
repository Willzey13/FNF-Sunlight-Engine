...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	/*
	* Lists all files in the characters directory
	* Extracts the file name (character) without the extension
	* Checks if the character is already in the list
	* Adds the new character to the list
	*/

	function addCharactersFromDirectory(directory:String):Void 
	{
    	if (FileSystem.exists(directory)) 
    	{
        	...
        	for (file in files) 
        	{
            	var characterName:String = file.substring(0, file.lastIndexOf('.hxs'));
	...
	function addSongUI():Void
	{
		...
		var charactersDirBoyfriend:String = 'assets/images/characters/' + _song.player1;
		var charactersDirOpponent:String = 'assets/images/characters/' + _song.player2;
		var charactersDirGirfriend:String = 'assets/images/characters/' + _song.gfVersion;

		addCharactersFromDirectory(charactersDirOpponent);
		addCharactersFromDirectory(charactersDirBoyfriend);
		addCharactersFromDirectory(charactersDirGirfriend);
	...
	var curZoom:Int = 2;
	var zoomList:Array<Float> = [2, 4, 6, 8, 12, 16, 24];
	var zoomTxt:FlxText;
	...
	function updateHeads():Void
	{
		...
		{
			leftIcon.changeIcon('bf', true);
			rightIcon.changeIcon('dad', false);
		}
		else
		{
			leftIcon.changeIcon('dad', false);
			rightIcon.changeIcon('bf', true);
		}
	}
	...
	function updateGrid():Void
	{
		...
		{
			Conductor.setBPM(_song.notes[curSection].bpm);
		...
		for (i in sectionInfo)
		{
			...
			var note:Note = new Note(daStrumTime, daNoteInfo % 4, "default", 0, false);
	...
	{
    	var sec:SwagSection = {
        	typeOfSection: 0, // ajuste o valor conforme necessário
        	lengthInSteps: lengthInSteps,
        	mustHitSection: true,
        	sectionNotes: [],
        	bpm: _song.bpm,
        	altAnim: false,
        	changeBPM: false // adicione changeBPM se necessário
    	};
