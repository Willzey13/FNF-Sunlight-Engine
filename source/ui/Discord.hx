...
{ 
	...
	{
		if(Saved.gameSettings.get("Discord RPC")) initialize();
		else if(isInitialized) shutdown();
	}
	
	public static function prepare()
	{
		if (!isInitialized && Saved.gameSettings.get("Discord RPC"))
			initialize();
	...
	public static function changePresence(?details:String = 'In the Menus', ?state:Null<String>, ?smallImageKey : String, ?hasStartTimestamp : Bool, ?endTimestamp: Float)
	{
		...
		presence.largeImageKey = 'sunlightlogo';
		presence.largeImageText = "Engine Version: " + CoolUtil.sunlightEngineVersion;
