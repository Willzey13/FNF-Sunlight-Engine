...
class OptionsSubstate extends MusicBeatSubstate
{
	...
	override public function create():Void
	{
		...
		// submenu group
		var submenuText = new Alphabet(0, 0, "Press any key to rebind", false);
		submenuText.screenCenter();
		submenuText.y -= 32;
		submenuGroup.add(submenuText);

		var submenuText2 = new Alphabet(0, 0, "Escape to Cancel", false);
	...
	private function generateOptions()
	{
		...
		for (controlString in Controls.allControls.keys())
		{
			arrayTemp[Controls.allControls.get(controlString)[1]] = controlString;
		}
		...
		for (i in 0...arrayTemp.length)
		{
			...
			var optionsText:Alphabet = new Alphabet(90, 320, arrayTemp[i].replace('_', ' '), true);
			optionsText.screenCenter();
			optionsText.y += (90 * (i - (arrayTemp.length / 2)));
			optionsText.targetY = i - curSelection;
			optionsText.changeX = false;
	...
	private function generateExtra(arrayTemp:Array<String>)
	{
		...
		for (i in 0...arrayTemp.length)
		{
			for (j in 0...2)
			{
				...
				if (Controls.allControls.exists(arrayTemp[i]))
					keyString = getStringKey(Controls.allControls.get(arrayTemp[i])[0][j]);

				var secondaryText:Alphabet = new Alphabet(0, 320, keyString, false);
				secondaryText.screenCenter();
				secondaryText.y += (90 * (i - (arrayTemp.length / 2)));
				secondaryText.targetY = i - curSelection;
				secondaryText.changeX = false;
	...
	{
		var left = Controls.justPressed("LEFT");
		var right = Controls.justPressed("RIGHT");
	...
	{
		//
		Saved.saveControls(); // for controls
		Saved.saveSettings(); // for offset
		super.close();
	}
	...
	private function subMenuControl()
	{
		...
		if (curSelection != keyOptions.length - 1)
		{
			...
			else if (FlxG.keys.justPressed.ANY)
			{
				...
				Controls.allControls.get(keyOptions.members[curSelection].text.replace(' ', '_'))[0][curHorizontalSelection] = checkKey;
